...
INTERFACE
    VAR_INPUT CONSTANT
        Buffer : POINTER TO dtFIFO_P_Element_REAL := 0; (*Externally allocated buffer. Must be in format ARRAY[0..N]. ! Block doesn't check for Null pointer*)
...
METHOD Enqueue:
    INTERFACE
        VAR_INPUT 
            Value : REAL; (*Value to add to queue*)
