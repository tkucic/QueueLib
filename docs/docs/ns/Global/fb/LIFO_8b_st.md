...
## Documentation for Function block LIFO_8b  

```pascal
//LIFO / Stack buffer of generic type that works with 8 bit values (based on BYTE). Pushing places new value on top of the stack and poping removes the element from the top. Strings and data structures are not supported.  
INTERFACE
    VAR_INPUT CONSTANT
...
METHOD Push:
    INTERFACE
        VAR_INPUT 
            Value : BYTE; (*Value to add to the stack*)
        END_VAR
    END_INTERFACE
