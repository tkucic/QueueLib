...
## Documentation for Function block FIFO_S_8b  

Standard FIFO buffer of generic type that works with 8 bit values (based on BYTE). Allows Enqueing if buffer is not full. After being filled, it has to be emptied completely before further equeueing. Strings and data structures are not supported.  
