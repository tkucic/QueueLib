...
INTERFACE
    VAR_INPUT CONSTANT
        Buffer : POINTER TO LDATE := 0; (*Externally allocated buffer. Must be in format ARRAY[0..N]. ! Block doesn't check for Null pointer*)
...
METHOD Push:
    INTERFACE
        VAR_INPUT 
            Value : LDATE; (*Value to add to the stack*)
        END_VAR
    END_INTERFACE
