...
## Documentation for Function block FIFO_UP_Dint  

```pascal
//Unordered Priority FIFO buffer. Allows Enqueing if buffer is not full. Dequeues the highest priority elements first.  
INTERFACE
    VAR_INPUT CONSTANT
        Buffer : POINTER TO dtFIFO_P_Element_DINT := 0; (*Externally allocated buffer. Must be in format ARRAY[0..N]. ! Block doesn't check for Null pointer*)
    ...
    END_VAR
END_INTERFACE
FUNCTION_BLOCK FIFO_UP_Dint:
    (***Developed entirely with methods***)
END_FUNCTION_BLOCK
METHOD Dequeue:
    INTERFACE
        VAR_OUTPUT 
            Value : DINT; (*Return value*)
...
METHOD Enqueue:
    INTERFACE
        VAR_INPUT 
            Value : DINT; (*Value to add to queue*)
            Priority : INT; (*Return priority*)
        END_VAR
    END_INTERFACE
